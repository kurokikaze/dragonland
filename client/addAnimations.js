...
const convertAction = action => {
	...
	switch(action.type) {
		...
		case ACTION_ATTACK: {
			return (action.source.owner !== window.playerId) ? [
				startAttackAnimation(action.source.id, action.target.id, (action.additionalAttackers && action.additionalAttackers.length) ? action.additionalAttackers[0].id : null,action.player), 
